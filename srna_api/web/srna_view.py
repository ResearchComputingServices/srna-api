...
def _compute_srnas_seq(user_filename, sequence_to_read, accession_number, format, shift, length, only_tags, blast, e_cutoff,identity_perc, follow_hits, shift_hits, gene_tags, locus_tags, client_session=None, tags_filename=None):
    try:
        ...
        print('Sequence file: %s' % sequence_to_read)
        ...
        sRNA_provider.export_output(user_filename, sequence_name,  description_seq, format, shift, shift_hits, length, e_cutoff, identity_perc, filepath_output, list_sRNA_recomputed, list_sRNA, gene_tags, locus_tags, tags_filename)
...
def _compute_srnas(self, user_filename, sequence_to_read, accession_number, format, shift, length, only_tags, blast, e_cutoff,identity_perc, follow_hits, shift_hits, gene_tags, locus_tags, client_session=None, tags_filename=None):
    try:
...
def compute_srnas():
    try:
        ...
        if only_tags:
            ...
            tags_filename = file_tags.filename
        #8. Create ouput folder per session id
        file_provider.create_folder(output_folder, client_session)

        #9. Call sRNA computation
        task = _compute_srnas.delay(file.filename,sequence_to_read, accession_number, format, shift, length, only_tags, blast, e_cutoff, identity_perc, follow_hits, shift_hits, gene_tags, locus_tags, client_session, tags_filename)
        ...
        # 9. Call sRNA computation
        #_compute_srnas_seq(file.filename, sequence_to_read, accession_number, format, shift, length,
        #                                only_tags, blast, e_cutoff, identity_perc, follow_hits, shift_hits, gene_tags,
        #                                locus_tags, client_session, tags_filename)

        #return jsonify({'Task_id': 1, 'Task_status': 'RECEIVED'}), 202, {}
...
@srna_bp.route("/get_template_file", methods=['GET'])
@crossdomain(origin='*')
def get_template_file():
    try:
        client_folder = "srna-data/templates" + '/'
        return file_provider.download_file(client_folder,'template_tags')
